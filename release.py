  ...
  def update_tags_in_source(self, tag_name):
    ...
    if ' 'not in tag_name and len(parts) == 3:
      ...
      for i in range(len(cmake_lines)):
        if "set(DATA_SERIAL_V_MAJOR" in cmake_lines[i]:
          cmake_lines[i] = f"set(DATA_SERIAL_V_MAJOR \"{version_maj}\")\n"
          print("updated version_maj...")
        if "set(DATA_SERIAL_V_MINOR" in cmake_lines[i]:
          cmake_lines[i] = f"set(DATA_SERIAL_V_MINOR \"{version_min}\")\n"
          print("updated version_min...")
        if "set(DATA_SERIAL_V_PATCH" in cmake_lines[i]:
          cmake_lines[i] = f"set(DATA_SERIAL_V_PATCH \"{version_patch}\")\n"
...
class docker_manager:
  def __init__(self, tag):
    ...
    self.full_tag_ = f"sparkplugbdemo.azurecr.io/data_serial:{tag}"
